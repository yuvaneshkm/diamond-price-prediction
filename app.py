...
if st.button("Predict Price"):
    try:
        details = DataPreparation(float(carat), str(cut), str(color), str(clarity), float(depth), float(table), float(x), float(y), float(z))
        details_df = details.custom_data()
        price = PredictionPipeline.predict(details_df)
